...
type User struct {
	...
	CurrentPointer string    `boil:"current_pointer" json:"current_pointer" toml:"current_pointer" yaml:"current_pointer"`
...
var UserTableColumns = struct {
	...
	Name:           "user.name",
	Token:          "user.token",
	CurrentPointer: "user.current_pointer",
...
var UserWhere = struct {
	...
	CurrentPointer whereHelperstring
	...
	CurrentPointer: whereHelperstring{field: "`user`.`current_pointer`"},
	Picture:        whereHelperstring{field: "`user`.`picture`"},
...
type userR struct {
	EmailFavs FavSlice `boil:"EmailFavs" json:"EmailFavs" toml:"EmailFavs" yaml:"EmailFavs"`
}
...
func (r *userR) GetEmailFavs() FavSlice {
	if r == nil {
		return nil
	}
	return r.EmailFavs
...
var (
	userAllColumns            = []string{"id", "email", "name", "token", "current_pointer", "picture", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"email", "name", "token", "current_pointer", "picture", "updated_at"}
...
// EmailFavs retrieves all the fav's Favs with an executor via email column.
func (o *User) EmailFavs(mods ...qm.QueryMod) favQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`fav`.`email`=?", o.Email),
	)

	return Favs(queryMods...)
}

// LoadEmailFavs allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadEmailFavs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load fav")
	}

	var resultSlice []*Fav
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice fav")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on fav")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for fav")
	}
	...
	if singular {
		object.R.EmailFavs = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.Email == foreign.Email {
				local.R.EmailFavs = append(local.R.EmailFavs, foreign)
				if foreign.R == nil {
					foreign.R = &favR{}
				}
...
// AddEmailFavs adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.EmailFavs.
// Sets related.R.EmailUser appropriately.
func (o *User) AddEmailFavs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Fav) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.EmailFavs = append(o.R.EmailFavs, related...)
	}
