...
var MigrationWhere = struct {
	...
	ID:        whereHelperint{field: "`migration`.`id`"},
	Version:   whereHelperstring{field: "`migration`.`version`"},
	CreatedAt: whereHelpertime_Time{field: "`migration`.`created_at`"},
}
...
var (
	migrationAllColumns            = []string{"id", "version", "created_at"}
	migrationColumnsWithoutDefault = []string{"version"}
	migrationColumnsWithDefault    = []string{"id", "created_at"}
...
func FindMigration(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Migration, error) {
	...
	query := fmt.Sprintf(
		"select %s from `migration` where `id`=?", sel,
	)
...
func (o *Migration) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `migration` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `migration` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, migrationPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for migration")
	}
...
func (o *Migration) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`migration`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `migration` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for migration")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for migration")
	}
...
func (o *Migration) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `migration` WHERE `id`=?"
...
func MigrationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `migration` where `id`=? limit 1)"
